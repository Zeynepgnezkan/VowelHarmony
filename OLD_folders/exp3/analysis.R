...
vh.preproc <- function(data_dir = ""){
  ...
  for(i in 1:length(dataALL)){#each subj
    ...
    data1 <- data1[ , -which(names(data1) %in% c("Welcome_screen.started","Welcome_screen.stopped","key_respwelcome.keys",
                                                  ...
                                                  "test_loop.thisTrialN","test_loop.thisN","anket.started","anket.stopped",
...
test_data <- vh.preproc(data_dir = "exp3/data/")
writexl::write_xlsx(test_data, "raw_data_v3.xlsx")
library(tidyverse)

accuracy <- test_data %>% group_by(participant) %>% summarise(meanacc = mean(key_respTest.corr, na.rm =T))
